...
/**
* findProgram - Finds a program in the path.
* @data: A pointer to the program's data.
* Return: 0 if success, error code otherwise.
*/
int findProgram(DataOfProgram *data)
{
	int i = 0, CodeReturn = 0;
	...
	for (i = 0; directoryIndex[i]; i++)
	{
		...
		CodeReturn = (checkFileExists(directoryIndex[i]));
		if (CodeReturn == 0 || CodeReturn == 126)
		{
			errno = 0;
			free(data->tokenArray[0]);
...
/**
* tokenizePath - Tokenizes the PATH variable into directory paths.
* @data: A pointer to the program's data.
